# KairosMd · 二次元极简 Markdown 阅读器

ヾ(≧▽≦*)o 一款用 Go + Wails + Vue 3 打造的桌面 Markdown **阅读器**。打开即读，主题换装，插件扩展，仅此而已。

## ✨ 特性

- **极简阅读**：打开即读，沉浸模式（F11），阅读位置记忆
- **二次元换装**：樱 / 宙 / 枫 / 墨 四款内置主题，热切换不重载；默认主题「枫」，与 logo 的暖色系呼应
- **主题特效**：樱→飘落花瓣、宙→闪烁星空与流星、枫→旋转红叶、墨→极淡墨尘，设置页或命令面板可随时开关
...
kairos.registerCommand({
  ...
  title: '打个招呼',
  hotkeys: ['alt+h'],
  run: () => kairos.ui.toast('你好呀~ (´▽`)', 'success')
})
